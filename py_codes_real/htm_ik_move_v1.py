...
class HTMIKMove(Node):
    def __init__(self, htm, duration_s, joint_state_topic='/franka/joint_states'):
        ...
        self.get_logger().info(f'Waiting for current joint state on {joint_state_topic}...')
...
def main():
    ...
    rclpy.init()
    node = HTMIKMove(htm, args.duration, joint_state_topic)
    rclpy.spin(node)
